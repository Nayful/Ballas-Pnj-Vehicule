...
ballasMenu = NativeUI.CreateMenu("Garage Ballas", "Liste des véhicules ballas", nil, nil, "shopui_title_ie_modgarage", "shopui_title_ie_modgarage")
_menuPool:Add(ballasMenu)
...
spawnCar = function(car)
    ...
    local vehicle = CreateVehicle(car, 88.42, -1967.38, 20.75, 317.29, true, false)
...
function AddBallasMenu(menu)
    ...
    voituremenu.SubMenu.OnItemSelect = function(menu, item)
        if item == sultan then 
            ESX.ShowAdvancedNotification("Ballas", "Actions Effectué", "Voici Ton Véhicule Man ! Fais gaffe à toi !", "CHAR_CARSITE", 1)
            Citizen.Wait(1)
            spawnCar("sultan")
...
AddBallasMenu(ballasMenu)
_menuPool:RefreshIndex()



local ballas = {
    {x = 84.83, y = -1967.37, z = 20.75},
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(ballas) do

            local plyCoords = GetEntityCoords(GetPlayerPed(-1), false)
            local dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, ballas[k].x, ballas[k].y, ballas[k].z)

            if dist <= 1.2 then
             if PlayerData.job2 ~= nil and PlayerData.job2.name == 'ballas' then 
                ESX.ShowHelpNotification("Appuyez sur ~INPUT_TALK~ pour intéragir avec ~b~Big D")
                if IsControlJustPressed(1,51) then 
                    latestPoint = ballas[k].type 
                    ballasMenu:Visible(not ballasMenu:Visible())
                end
            end
...
local v1 = vector3(84.14, -1966.75, 21.93)   -- Le nom au dessus du PNJ
...
Citizen.CreateThread(function()
    while true do
        ...
        if Vdist2(GetEntityCoords(PlayerPedId(), false), v1) < distance then
            Draw3DText(v1.x,v1.y,v1.z, "Big D")
		end
	end
end)

Citizen.CreateThread(function()
    local hash = GetHashKey("csb_ballasog")
    while not HasModelLoaded(hash) do
    RequestModel(hash)
    Wait(20)
    end
    ped = CreatePed("PED_TYPE_CIVMALE", "csb_ballasog", 84.14, -1966.75, 19.94, 230.09, false, true) --Emplacement du PEDS
